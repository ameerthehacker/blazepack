...
var PreviewManager = /** @class */ (function () {
    function PreviewManager(selector, sandboxInfo, options) {
        ...
        this.getManagerTranspilerContext = function () {
            return new Promise(function (resolve) {
                var listener = _this.listen(function (message) {
        ...
        if (!this.iframe.getAttribute('allow')) {
            this.iframe.setAttribute('allow', 'accelerometer; ambient-light-sensor; autoplay; camera; encrypted-media; geolocation; gyroscope; hid; microphone; midi; payment; usb; vr; xr-spatial-tracking');
        }
        this.iframe.src = this.bundlerURL;
        this.listener = listen(function (mes) {
            if (mes.type !== 'initialized' && mes.$id && mes.$id !== _this.id) {
            ...
            switch (mes.type) {
                case 'initialized': {
                    if (_this.iframe) {
                        if (_this.iframe.contentWindow) {
                            registerFrame(_this.iframe.contentWindow, _this.bundlerURL, _this.id);
                ...
                case 'action': {
                    if (mes.action === 'show-error') {
                        var title = mes.title, path = mes.path, message = mes.message, line = mes.line, column = mes.column;
                        _this.errors = __spreadArrays(_this.errors, [
                            { title: title, path: path, message: message, line: line, column: column },
                        ]);
                    }
                    break;
                }
                case 'state': {
                    _this.managerState = mes.state;
                    break;
                }
                default: 
            }
    ...
    PreviewManager.prototype.updatePreview = function (sandboxInfo, isInitializationCompile) {
    ...
    PreviewManager.prototype.listen = function (listener) {
        var _this = this;
        return listen(function (msg) {
            if (msg.$id !== _this.id) {
                return;
            }
            listener(msg);
